...
(0, ava_1.default)("invokes middleware", async (t) => {
...
(0, ava_1.default)("invokes middleware in order", async (t) => {
    const recorder = [];
    const { queue, jobtype } = await (0, _helper_1.push)();
    const worker = new worker_1.Worker({
        concurrency: 1,
...
(0, ava_1.default)(".use() adds middleware to the stack", (t) => {
    const worker = new worker_1.Worker();
    const mmw = () => { };
    worker.use(mmw);
    t.is(worker.middleware[0], mmw, "middleware function not added to .middleware");
});
(0, ava_1.default)("middleware context is passed to job thunk", async (t) => {
    const { queue, jobtype } = await (0, _helper_1.push)({ args: [1] });
