...
const mocked = async (fn) => {
    const server = (0, exports.mockServer)();
...
const randQueue = (label = "test") => {
    return `${label}-${(0, uuid_1.v4)().slice(0, 6)}`;
};
exports.randQueue = randQueue;
const createJob = (...args) => {
    return {
        jobtype: "testJob",
        queue: (0, exports.randQueue)(),
        args,
    };
};
...
const push = async ({ args, queue, jobtype, } = {}) => {
    ...
    job.queue = queue || (0, exports.randQueue)();
    job.args = args || [];
    await job.push();
    client.close();
    return job;
